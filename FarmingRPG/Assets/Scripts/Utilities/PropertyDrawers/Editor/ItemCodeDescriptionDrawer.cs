...
public class ItemCodeDescriptionDrawer : PropertyDrawer
{
    ...
    {
        // Using BeginProperty on the parent property means that prefab override logic works on the entire property
        EditorGUI.BeginProperty(position, label, property);

        if(property.propertyType == SerializedPropertyType.Integer)
        {
            EditorGUI.BeginChangeCheck(); // Start to check for changed values

            // Draw Item
            var newValue = EditorGUI.IntField(new Rect(position.x, position.y, position.width, position.height / 2), label,
                                                property.intValue);
    ...
    private string GetItemDescription(int itemCode)
    {
        ...
        so_itemList = AssetDatabase.LoadAssetAtPath("Assets/ScriptableObjectAssets/Item/so_ItemList.asset",
